...
fn execute_command(
  ...
  // Strip the rustc configuration, otherwise in the helper it will attempt to compile the
  // program with the toolchain of the normal build process, rather than the Athena toolchain.
...
fn copy_elf_to_output_dir(
  ...
  // 1. --elf_name flag
  // 2. --binary flag + -elf suffix (defaults to package name)
