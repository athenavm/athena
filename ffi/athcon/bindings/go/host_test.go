...
func TestVerify(t *testing.T) {
	...
	var walletAddress Address
	{
		...
		encoded, err := scale.Marshal(executionPayload)
		require.NoError(t, err)
		result, err := vm.Execute(host, Frontier, Call, 1, 10000000, principal, principal, encoded, 0, WALLET_TEST)
		require.NoError(t, err)
		require.Len(t, result.Output, 24)
	...
	encoded, err := scale.Marshal(executionPayload)
	require.NoError(t, err)
	result, err := vm.Execute(host, Frontier, Call, 1, 1000000000, principal, principal, encoded, 0, WALLET_TEST)
	require.NoError(t, err)
