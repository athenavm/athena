...
#define DLL_GET_CREATE_FN(handle, name) (athcon_create_fn)(uintptr_t) GetProcAddress(handle, name)
...
#define DLL_GET_CREATE_FN(handle, name) (athcon_create_fn)(uintptr_t) dlsym(handle, name)
...
static enum athcon_loader_error_code set_error(enum athcon_loader_error_code error_code,
                                               const char *format,
                                               ...)
{
  va_list args;
...
athcon_create_fn athcon_load(const char *filename, enum athcon_loader_error_code *error_code)
{
  ...
  enum athcon_loader_error_code ec = ATHCON_LOADER_SUCCESS;
  athcon_create_fn create_fn = NULL;

  if (!filename)
  {
    ec = set_error(ATHCON_LOADER_INVALID_ARGUMENT, "invalid argument: file name cannot be null");
    goto exit;
  }
  ...
  if (length == 0)
  {
    ec = set_error(ATHCON_LOADER_INVALID_ARGUMENT, "invalid argument: file name cannot be empty");
    goto exit;
  }
  else if (length > PATH_MAX_LENGTH)
  {
    ec = set_error(ATHCON_LOADER_INVALID_ARGUMENT,
  ...
  if (!handle)
  {
    ...
    if (last_error_msg)
      ec = ATHCON_LOADER_CANNOT_OPEN;
    else
      ec = set_error(ATHCON_LOADER_CANNOT_OPEN, "cannot open %s", filename);
    goto exit;
  }

  // Create name buffer with the prefix.
  const char prefix[] = "athcon_create_";
  ...
  if (!create_fn)
    create_fn = DLL_GET_CREATE_FN(handle, "athcon_create");

  if (!create_fn)
  {
    DLL_CLOSE(handle);
    ec = set_error(ATHCON_LOADER_SYMBOL_NOT_FOUND, "ATHCON create function not found in %s",
                   filename);
  }
...
struct athcon_vm *athcon_load_and_create(const char *filename, enum athcon_loader_error_code *error_code)
{
  ...
  athcon_create_fn create_fn = athcon_load(filename, error_code);
  ...
  struct athcon_vm *vm = create_fn();
  if (!vm)
  {
    ec = set_error(ATHCON_LOADER_VM_CREATION_FAILURE, "creating ATHCON VM of %s has failed",
                   filename);
    goto exit;
  }

  if (!athcon_is_abi_compatible(vm))
  {
    ec = set_error(ATHCON_LOADER_ABI_VERSION_MISMATCH,
                   "ATHCON ABI version %d of %s mismatches the expected version %d",
                   vm->abi_version, filename, ATHCON_ABI_VERSION);
    athcon_destroy(vm);
    vm = NULL;
    goto exit;
  }
...
struct athcon_vm *athcon_load_and_configure(const char *config, enum athcon_loader_error_code *error_code)
{
  enum athcon_loader_error_code ec = ATHCON_LOADER_SUCCESS;
  struct athcon_vm *vm = NULL;
  ...
  vm = athcon_load_and_create(path, error_code);
  if (!vm)
    return NULL;
  ...
  while (strlen(options) != 0)
  {
    ...
    {
      ec = set_error(ATHCON_LOADER_INVALID_OPTION_NAME, "%s (%s) does not support any options",
                     vm->name, path);
      goto exit;
    }
    ...
    enum athcon_set_option_result r = vm->set_option(vm, name, option);
    switch (r)
    {
    case ATHCON_SET_OPTION_SUCCESS:
      break;
    case ATHCON_SET_OPTION_INVALID_NAME:
      ec = set_error(ATHCON_LOADER_INVALID_OPTION_NAME, "%s (%s): unknown option '%s'",
                     vm->name, path, name);
      goto exit;
    case ATHCON_SET_OPTION_INVALID_VALUE:
      ec = set_error(ATHCON_LOADER_INVALID_OPTION_VALUE,
